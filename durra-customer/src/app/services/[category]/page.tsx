...
// عناوين احتياطية لو ما لقينا الفئة في Firebase
...
export default function ServiceCategoryPage() {
  ...
  useEffect(() => {
    if (!category) return;
    Promise.all([
      getDocs(query(collection(db, "providers"), where("approved", "==", true), where("type", "==", category), orderBy("rating", "desc"))),
      getDocs(query(collection(db, "serviceCategories"), where("value", "==", category))),
    ]).then(([provSnap, catSnap]) => {
      setProviders(provSnap.docs.map(d => ({ id: d.id, ...d.data() })));
      if (!catSnap.empty) setCategoryTitle(catSnap.docs[0].data().title);
      else setCategoryTitle(FALLBACK_LABELS[category] || "الخدمات");
      setLoading(false);
    }).catch(() => {
      setCategoryTitle(FALLBACK_LABELS[category] || "الخدمات");
      setLoading(false);
    });
  }, [category]);
