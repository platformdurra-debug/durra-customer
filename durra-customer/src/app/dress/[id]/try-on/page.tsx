...
export default function TryOnPage() {
  ...
  const handleTryOn = async () => {
    ...
    try {
      ...
      // ── Fashn.ai API ──
      // أضيفي NEXT_PUBLIC_FASHN_API_KEY في .env.local
      const apiKey = process.env.NEXT_PUBLIC_FASHN_API_KEY;

      if (!apiKey || apiKey === "YOUR_FASHN_API_KEY") {
        // وضع التجريب — نرجع صورة وهمية
        await new Promise(r => setTimeout(r, 3000));
        clearInterval(timer);
        setProgress(100);
        // في وضع التطوير نعرض صورة الفستان كنتيجة مؤقتة
  ...
  return (
    <div className="min-h-screen pb-10" style={{ background: "linear-gradient(160deg, #1a0a00, #2c1810)" }}>
      ...
      <div className="px-5">
        ...
        {/* Dress preview */}
        {dress && (
          <div className="flex gap-3 items-center p-3 rounded-2xl mb-5"
            style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(201,169,110,0.2)" }}>
            <img src={dress.images?.[0]} alt={dress.name}
              className="w-14 h-18 object-cover rounded-xl flex-shrink-0" style={{ height: 70 }} />
            <div>
        ...
        {stage === "upload" && (
          <div>
            ...
            <div onClick={() => fileInputRef.current?.click()}
              className="rounded-2xl flex flex-col items-center justify-center cursor-pointer transition-all mb-4"
              style={{
                height: userPhotoUrl ? "auto" : 220,
                background: "rgba(255,255,255,0.04)",
                border: `2px dashed ${userPhotoUrl ? "#C9A96E" : "rgba(255,255,255,0.15)"}`,
                padding: userPhotoUrl ? 0 : "40px 20px",
              }}>
              {userPhotoUrl ? (
                <img src={userPhotoUrl} alt="صورتك" className="w-full rounded-2xl object-cover"
                  style={{ maxHeight: 350 }} />
              ) : (
                <>
            ...
            {userPhotoUrl && (
              <div className="flex gap-3">
                <button onClick={reset} className="flex-1 py-3 rounded-2xl font-bold"
                  style={{ background: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.6)", fontFamily: "Tajawal, sans-serif" }}>
                  تغيير الصورة
                </button>
          ...
          <div className="text-center py-10">
            <div className="relative w-32 h-32 mx-auto mb-6">
              ...
              <svg className="absolute inset-0 w-full h-full" style={{ animation: "spin 2s linear infinite" }}
                ...
                <circle cx="50" cy="50" r="44" fill="none" stroke="#C9A96E" strokeWidth="4"
            ...
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", marginBottom: 24 }}>
              الذكاء الاصطناعي يضع الفستان عليك
            </div>
            ...
            <div className="mt-6 flex flex-col gap-2">
              {["تحليل صورتك...", "تجهيز الفستان...", "دمج الصور بالذكاء الاصطناعي...", "تحسين النتيجة..."].map((step, i) => (
                <div key={step} className="flex items-center gap-3 px-4 py-2 rounded-xl text-right"
                  style={{ background: progress > i * 25 ? "rgba(201,169,110,0.1)" : "rgba(255,255,255,0.03)", opacity: progress > i * 20 ? 1 : 0.3 }}>
        ...
        {stage === "result" && (
          <div>
            ...
            {!process.env.NEXT_PUBLIC_FASHN_API_KEY && (
              <div className="rounded-xl p-3 mb-4" style={{ background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.2)" }}>
                <div style={{ fontSize: 12, color: "#F59E0B" }}>
                  ⚠️ وضع التطوير — أضيفي NEXT_PUBLIC_FASHN_API_KEY للنتائج الحقيقية
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={downloadResult} className="w-full py-3 rounded-2xl font-bold"
                style={{ background: "rgba(255,255,255,0.08)", color: "#fff", fontFamily: "Tajawal, sans-serif" }}>
                📥 حفظ الصورة
              </button>
              <button onClick={reset} className="w-full py-3 rounded-2xl font-bold flex items-center justify-center gap-2"
                style={{ background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.5)", fontFamily: "Tajawal, sans-serif" }}>
                <RefreshCw size={14} />
                جرّبي صورة ثانية
              </button>
            </div>
